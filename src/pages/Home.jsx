...
const Home = () => {
  ...
  const handleDonateClick = (requestId) => {
    navigate(`/${username}/donate/${requestId}`);
  };
          ...
          >
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-semibold text-gray-900">{userRequests}</p>
              </div>
            </div>
          </div>
          ...
          >
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-semibold text-gray-900">{urgentNeeds}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white shadow rounded-lg overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-center">
            ...
            <div className="mt-3 sm:mt-0 flex flex-wrap gap-2">
              <button
                onClick={() => setActiveTab('all')}
                className={`px-3 py-1 text-sm rounded-md transition-colors ${activeTab === 'all' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
              >
                All
              </button>
              <button
                onClick={() => setActiveTab('critical')}
                className={`px-3 py-1 text-sm rounded-md transition-colors ${activeTab === 'critical' ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
              >
                Critical
              </button>
              <button
                onClick={() => setActiveTab('urgent')}
                className={`px-3 py-1 text-sm rounded-md transition-colors ${activeTab === 'urgent' ? 'bg-yellow-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
              >
                Urgent
              </button>
              <button
                onClick={() => setActiveTab('normal')}
                className={`px-3 py-1 text-sm rounded-md transition-colors ${activeTab === 'normal' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
              >
                Normal
              </button>
            </div>
          ...
          {error ? (
            ...
            <div className="p-6 text-center">
              ...
              <p className="mt-2 text-gray-500">No blood requests found</p>
            </div>
          ) : (
                    ...
                    <div className="mt-4 md:mt-0 md:ml-4 flex flex-col sm:flex-row md:flex-col lg:flex-row gap-2">
                      ...
                      {user?.isDonor && (
                        <button
                          ...
                          className="px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-purple-600 hover:bg-purple-700 transition-colors"
                        >
                          Donate
                        </button>
                      )}
